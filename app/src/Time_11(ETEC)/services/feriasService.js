...
function calcularFerias(salarioBruto, diasDireito = 30, diasConcedidos = 30) {
  ...
  const umTerco = Number.parseFloat((valorFerias / 3).toFixed(2));
  const totalBruto = Number.parseFloat((valorFerias + umTerco).toFixed(2));
  ...
  const totalLiquido = Number.parseFloat((totalBruto - inss).toFixed(2));

  return {
    salarioBruto,
    diasConcedidos,
    valorFerias: Number.parseFloat(valorFerias.toFixed(2)),
