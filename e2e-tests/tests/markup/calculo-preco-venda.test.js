...
async function main() {
    ...
    try {
        ...
        await tiraFoto('Markup-preco-venda-01-aberto');
        ...
        await tiraFoto('Markup-preco-venda-02-preenchido');

        await driver.findElement(By.id('btnCalcular')).click();

        await driver.sleep(1000);

        await tiraFoto('Markup-preco-venda-03-calculado');
        ...
        if (!resultado.includes('181.82')) {
            throw new Error(`Resultado inesperado: ${resultado}`);
        }

        await tiraFoto('Markup-preco-venda-04-resultado');

        console.log('✅ Cálculo de preço de venda validado com sucesso');
...
main()
    .then(() => console.log('(Markup) Teste de cálculo de preço de venda concluído'))
    .catch(err => {
        console.error(err);
        process.exit(1);
    });